...
# =============================================================================
# Step 1/8 — 创建 conda 环境
# =============================================================================
echo "=== [Step 1/8] 检查并创建 conda 环境 ${ENV_NAME} ==="
...
# =============================================================================
# Step 2/8 — 获取环境内 pip / python 绝对路径
# =============================================================================
echo ""
echo "=== [Step 2/8] 获取环境内 pip / python 路径 ==="
...
# =============================================================================
echo ""
echo "=== [Step 3/8] 安装 PyTorch（torch>=2.4.0, torchvision, torchaudio），CUDA=${TORCH_CUDA_TAG} ==="
...
# =============================================================================
# Step 4/8 — 安装 flash_attn（需要 GPU 节点源码编译）
# =============================================================================
echo ""
echo "=== [Step 4/8] 安装 flash_attn（需要 GPU 节点源码编译）==="
set +e
...
# =============================================================================
# Step 5/8 — 安装 lingbot-world 其余依赖（排除 torch/torchvision/torchaudio/flash_attn）
# =============================================================================
echo ""
echo "=== [Step 5/8] 安装 lingbot-world 其余依赖（来自 requirements.txt，已排除 torch 系列和 flash_attn）==="
...
# =============================================================================
# Step 6/8 — editable 安装 lingbot-world（wan 包）
# =============================================================================
echo ""
echo "=== [Step 6/8] 以 editable 方式安装 lingbot-world（pip install -e）==="
...
# =============================================================================
# Step 7/8 — 安装 memory_module 额外依赖
# =============================================================================
echo ""
echo "=== [Step 7/8] 安装 memory_module 额外依赖：einops, wandb, pytest ==="
...
# =============================================================================
# Step 8/8 — 安装 VBench 评测依赖
# =============================================================================
echo ""
echo "=== [Step 8/8] 安装 VBench 评测依赖 ==="

"${PIP}" install vbench
echo "  [OK] vbench 安装完成。"

"${PIP}" install scenedetect
echo "  [OK] scenedetect（VBench-Long 场景分割）安装完成。"

set +e
echo "[INFO] 尝试安装 detectron2（登录节点无 GPU 时会失败，属正常现象）..."
"${PIP}" install 'detectron2@git+https://github.com/facebookresearch/detectron2.git'
DETECTRON2_EXIT=$?
set -e

if [ ${DETECTRON2_EXIT} -ne 0 ]; then
    echo ""
    echo "[WARN] detectron2 安装失败（登录节点无 GPU，属正常现象）。"
    echo "  VBench 评测中 imaging_quality 维度将不可用，其余 5 个维度不受影响。"
    echo "  请在 GPU 节点上手动安装："
    echo "    conda activate ${ENV_NAME}"
    echo "    pip install 'detectron2@git+https://github.com/facebookresearch/detectron2.git'"
    echo "[INFO] 继续安装..."
fi
...
try:
    ...
    print('[WARN] vbench not installed')
print()
