...
Loads base act model + LoRA weights, generates video from image + action controls.

Usage:
    torchrun --nproc_per_node=8 inference_csgo.py \
        --ckpt_dir /home/nvme02/lingbot-world/models/lingbot-world-base-act/ \
        --lora_path /home/nvme02/lingbot-world/output/csgo_lora_r32/epoch_50/lora_weights.pth \
        --image /home/nvme02/lingbot-world/datasets/csgo_processed/val/clips/<clip>/image.jpg \
        --action_path /home/nvme02/lingbot-world/datasets/csgo_processed/val/clips/<clip>/ \
        --prompt "First-person view of CS:GO gameplay on de_nuke" \
...
def main():
    ...
    parser.add_argument("--ft_model_dir", type=str, default=None,
                        help="Path to fully fine-tuned low_noise_model directory (overrides ckpt_dir's low_noise_model)")
    ...
    # If LoRA weights provided, we need to:
    ...
    if args.lora_path:
        ...
        # Load base low_noise_model with control_type='act' (control_dim=7)
        # This matches how WanI2V loads the model (image2video.py lines 93-96, 115-116)
        model = WanModel.from_pretrained(
            args.ckpt_dir, subfolder="low_noise_model",
            torch_dtype=torch.bfloat16, control_type="act",
        )

        # Load LoRA state dict to detect target modules
        lora_state = torch.load(args.lora_path, map_location="cpu")
        target_modules = set()
        for key in lora_state.keys():
            # Extract module name from LoRA key like "blocks.0.self_attn.q.lora_A.default.weight"
            parts = key.split(".")
        ...
        # Map keys if needed (lora_A.weight → lora_A.default.weight)
        mapped_state = {}
        for key, val in lora_state.items():
        ...
        logging.info(f"Loaded LoRA weights: {len(mapped_state)} keys, missing={len(result.missing_keys)}, unexpected={len(result.unexpected_keys)}")

        # Now save the merged model temporarily for inference
        # Or better: monkey-patch the WanI2V to use our model
        # For simplicity, save merged low_noise_model and point to it
        ...
        args.ckpt_dir = tmp_dir
        del model, lora_state, mapped_state
        torch.cuda.empty_cache()

    elif args.ft_model_dir:
        # For full fine-tuned model: create temp dir with symlinks
        import tempfile
        tmp_dir = tempfile.mkdtemp(prefix='act_')
    ...
    # Now use standard LingBot inference
